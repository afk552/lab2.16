...
def validate_json(json_data):
    ...
    class PeopleSchema(Schema):
        name = fields.Str(validate=validate.Regexp(name_reg))
        pnumber = fields.Str(validate=validate.Regexp(pnumber_reg))
        birth = fields.Str(validate=validate.Regexp(birth_reg))

    try:
        people = PeopleSchema().load(json_data, many=True)
        return people
...
def load_workers(file_name):
    ...
    with open(file_name, "r", encoding="utf-8") as f_in:
        ...
        if is_valid:
            ...
            print("Задан некорректный файл!", file=sys.stderr)
            return []
